...
/* Due operazioni finte: al registro interessa solo se il puntatore e' nullo o
   no, non cosa fanno. */
static int finta_read(struct chardev *d, void *buf, uint32_t n)
{
    (void)d; (void)buf; (void)n;
    return 0;
}

static int finta_write(struct chardev *d, const void *buf, uint32_t n)
{
    (void)d; (void)buf; (void)n;
    return (int)n;
}
...
static struct chardev fai(const char *nome, uint16_t major, uint16_t minor,
...
{
    struct chardev d = fai("console", 5, 1, 0, 1);

    chardev_init();

    check("il registro parte vuoto", chardev_count() == 0);
    check("la prima iscrizione ritorna 0", chardev_register(&d) == 0);
    check("chardev_count passa a 1", chardev_count() == 1);
}
...
/* Il controllo che distingue una copia da un alias, e l'unico modo di farlo
   ...
   Se chardev_register conservasse il puntatore invece di copiare, il registro
...
{
    struct chardev d = fai("aaa", 7, 7, 1, 1);
    struct chardev *reg;
...
static void test_nome_al_limite(void)
{
    ...
    /* Lo stesso caso, ma con major = 0 — e questo e' quello che conta.
       ...
       registro un nome senza terminatore. Da quel momento chardev_find legge
...
{
    struct chardev d = fai("", 6, 6, 1, 1);

    chardev_init();

    check("un nome vuoto e' rifiutato", chardev_register(&d) == -1);
}

static void test_senza_operazioni(void)
{
    struct chardev d = fai("muto", 8, 8, 0, 0);
...
{
    struct chardev a = fai("console", 5, 1, 0, 1);
    struct chardev b = fai("ttyS0", 4, 64, 0, 1);
    struct chardev c = fai("kbd", 13, 0, 1, 0);
...
static void test_find(void)
{
    ...
    d = chardev_find("console");
    check("chardev_find trova console", d != 0 && d->major == 5);

    d = chardev_find("ttyS0");
    check("chardev_find trova ttyS0", d != 0 && d->minor == 64);

    d = chardev_find("kbd");
    check("chardev_find trova kbd", d != 0 && d->major == 13);

    check("chardev_find su un nome assente ritorna 0",
          chardev_find("pippo") == 0);

    /* La corrispondenza e' esatta. Un confronto sui primi N caratteri, o una
       strpos usata come "contiene", troverebbero console. */
    check("chardev_find non accetta un prefisso",
          chardev_find("cons") == 0);
}

static void test_by_id(void)
{
    struct chardev a = fai("uno", 5, 1, 1, 0);
    struct chardev b = fai("due", 5, 2, 1, 0);   /* stesso major! */
    struct chardev *d;
    ...
    d = chardev_by_id(5, 1);
    check("chardev_by_id trova per coppia", d != 0 && same(d->name, "uno"));

    /* Il controllo che prende un confronto sul solo major: due dispositivi che
       differiscono per il minor devono restare distinti. */
    d = chardev_by_id(5, 2);
    check("chardev_by_id distingue due minor sotto lo stesso major",
          d != 0 && same(d->name, "due"));

    check("chardev_by_id su una coppia assente ritorna 0",
          chardev_by_id(5, 3) == 0);
}
...
static void test_at(void)
{
    ...
    check("chardev_at restituisce i tre in ordine di iscrizione",
    ...
    /* Due estremi: sotto zero legge i byte prima dell'array, sopra il conteggio
       ...
       puntatore nullo. */
    check("chardev_at fuori intervallo ritorna 0",
          chardev_at(-1) == 0 && chardev_at(chardev_count()) == 0 &&
          chardev_at(MAX_DEVICES) == 0);
}
